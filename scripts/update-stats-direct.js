/**
 * Idempotent points sync + pending Sara/Husnain adjustments.
 * Prefer: npx tsx scripts/update-stats-direct.ts
 * Or POST /api/admin/sync-points with x-admin-auth: true
 */
...
async function syncAllUsers() {
  ...
  for (const user of users || []) {
    ...
    const needsUpdate =
      after.total > (Number(pointsRow.total_points || 0) || 0) ||
