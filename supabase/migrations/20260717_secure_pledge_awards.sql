-- Make pledge point awards idempotent, atomic, and server-controlled.

ALTER TABLE public.pledges
  ADD COLUMN IF NOT EXISTS submission_id UUID,
  ADD COLUMN IF NOT EXISTS points_awarded INTEGER,
  ADD COLUMN IF NOT EXISTS award_status TEXT NOT NULL DEFAULT 'legacy';
...
-- Pledge rows affect public leaderboards, so authenticated clients must not be
-- able to forge rows or point-audit fields directly.
